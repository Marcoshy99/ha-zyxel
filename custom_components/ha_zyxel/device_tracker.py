"""Device tracker platform for Zyxel routers."""
...
async def async_setup_entry(
    ...
    """Create a tracker for every LAN host."""
    ...
    if track_all:
        ...
        for reg in er.async_entries_for_config_entry(registry, entry.entry_id):
            if reg.domain == "device_tracker" and reg.disabled_by == er.RegistryEntryDisabler.INTEGRATION:
...
class ZyxelDeviceTracker(CoordinatorEntity, ScannerEntity):
    ...
    def __init__(
        self, coordinator, entry: ConfigEntry, mac: str, consider_home: int, track_all: bool = False
    ) -> None:
