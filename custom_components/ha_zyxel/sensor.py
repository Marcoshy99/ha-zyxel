    ...
    def _discover_clients() -> None:
        ...
        for mac in lan_hosts(coordinator):
            if mac not in tracked:
                tracked.add(mac)
                new.extend(
                    ZyxelClientAttrSensor(coordinator, entry, mac, spec)
...
class _ZyxelClientSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, entry: ConfigEntry, mac: str):
        super().__init__(coordinator)
        self._mac = mac
...
class ZyxelClientAttrSensor(_ZyxelClientSensor):
    ...
    def __init__(self, coordinator, entry: ConfigEntry, mac: str, spec: dict):
        super().__init__(coordinator, entry, mac)
        self._value_fn = spec["fn"]
        self._attr_unique_id = f"{entry.entry_id}_{mac}_{spec['id']}"
